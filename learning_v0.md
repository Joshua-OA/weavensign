# learning_v0.md — weavensign build log (entries 001-010)
...
Split across `learning_v0.md`, `learning_v1.md`, ... once any one file would exceed
~200 lines — each file covers a contiguous range of entry numbers (see the header of
each file), never rewritten or renumbered once written. Entry numbers are the stable
citation key across the whole codebase ("see learning_vN.md #NNN"), not line numbers
or filenames — if you don't know which file has a given #NNN, check each file's header
range, or grep `### NNN` across `learning_v*.md`.
